...
extends Module {
    ...
    public static Map<Integer, ItemStack> getInventorySlots(int n, int n2) {
        ...
        while (++current <= last) {
            fullInventorySlots.put(current, (ItemStack)ModuleHotbarFill.mc.player.inventoryContainer.getInventory().get(current));
        }
    ...
    public int findCompatibleInventorySlot(ItemStack itemStack) {
        ...
        for (Map.Entry<Integer, ItemStack> entry : ModuleHotbarFill.getInventory().entrySet()) {
            ...
            if (inventoryStack.isEmpty() || inventoryStack.getItem() == Items.AIR || !this.isCompatibleStacks((ItemStack)hotbarStack, inventoryStack) || smallestStackSize <= (currentStackSize = ((ItemStack)ModuleHotbarFill.mc.player.inventoryContainer.getInventory().get(entry.getKey().intValue())).getCount())) continue;
    ...
    public boolean isCompatibleStacks(ItemStack itemStack, ItemStack itemStack2) {
        ...
        if (stack1.getItem() instanceof ItemBlock) {
            if (stack2.getItem() instanceof ItemBlock) {
                ...
                Block block = ((ItemBlock)stack2.getItem()).getBlock();
            }
        }

...
extends Module {
    ...
    public float filterPosition(BlockPos blockPos) {
        void position;
        if (yCheck.getValue() && position.getY() != ModuleNewCrystalBasePlace.mc.player.getPosition().getY()) {
            return Float.intBitsToFloat(Float.floatToIntBits(-7.911857f) ^ 0x7F7D2DEF);
        }
        if (!ModuleNewCrystalBasePlace.mc.world.getBlockState((BlockPos)position).getBlock().isReplaceable((IBlockAccess)ModuleNewCrystalBasePlace.mc.world, (BlockPos)position) || !ModuleNewCrystalBasePlace.mc.world.getBlockState(position.up()).getBlock().isReplaceable((IBlockAccess)ModuleNewCrystalBasePlace.mc.world, position.up())) {
        ...
        if (ModuleNewCrystalBasePlace.mc.player.getDistanceSq((BlockPos)position) > (double)MathUtils.square(placeRange.getValue().floatValue())) {
            return Float.intBitsToFloat(Float.floatToIntBits(-7.7399526f) ^ 0x7F77ADB1);
        }
        float targetDamage = DamageUtils.calculateDamage((double)position.getX() + Double.longBitsToDouble(Double.doubleToLongBits(3.859775411443625) ^ 0x7FEEE0D1EE507148L), (double)position.getY() + Double.longBitsToDouble(Double.doubleToLongBits(11.291667435420424) ^ 0x7FD695556F20E0B9L), (double)position.getZ() + Double.longBitsToDouble(Double.doubleToLongBits(3.287754204373365) ^ 0x7FEA4D5213888F92L), (EntityLivingBase)this.target);
        float selfDamage = DamageUtils.calculateDamage((double)position.getX() + Double.longBitsToDouble(Double.doubleToLongBits(3.0321794875384644) ^ 0x7FE841E751B4A351L), (double)position.getY() + Double.longBitsToDouble(Double.doubleToLongBits(5.216471668479226) ^ 0x7FE4DDAABFC283ECL), (double)position.getZ() + Double.longBitsToDouble(Double.doubleToLongBits(70.4361477151085) ^ 0x7FB19BE9D81B276FL), (EntityLivingBase)ModuleNewCrystalBasePlace.mc.player);

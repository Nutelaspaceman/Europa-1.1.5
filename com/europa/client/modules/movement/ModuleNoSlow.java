...
extends Module {
    ...
    public void onInput(InputUpdateEvent inputUpdateEvent) {
        if (noSlow.getValue() && ModuleNoSlow.mc.player.isHandActive() && !ModuleNoSlow.mc.player.isRiding()) {
            event.getMovementInput().moveStrafe *= Float.intBitsToFloat(Float.floatToIntBits(1.5738807f) ^ 0x7F6974EC);
            event.getMovementInput().moveForward *= Float.intBitsToFloat(Float.floatToIntBits(0.3323823f) ^ 0x7E0A2E03);
        }
    }

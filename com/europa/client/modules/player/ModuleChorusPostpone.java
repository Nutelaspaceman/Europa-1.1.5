...
extends Module {
    ...
    public void finishEating(LivingEntityUseItemEvent.Finish finish) {
        void event;
        if (event.getEntity() == ModuleChorusPostpone.mc.player && event.getResultStack().getItem().equals((Object)Items.CHORUS_FRUIT)) {
    ...
    public void onSend(EventPacket.Send send) {
        block1: {
            ...
            if (!(event.getPacket() instanceof CPacketPlayer) || !this.ateChorus || this.delay2 >= this.sDelay.getValue().intValue()) break block1;
